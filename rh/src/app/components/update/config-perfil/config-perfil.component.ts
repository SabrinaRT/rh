...
export class ConfigPerfilComponent implements OnInit {
    ...
    this.siscrhService
      ...
      .subscribe((data: any) => {
        ...
        this.mensagemCodataResetSenha =
          ...
          this.dadosProfissionais.email_institucional;
      });
  }
  ...
  enviarEmailRhDesativar() {
    this.toastr.warning(
      'Estamos enviando a solicitação para a equipe GINF...',
      'Aguarde!'
    );
    this.siscrhService.enviarEmailRhDesativar(this.IDColab).subscribe(
      (data: any) => {
  ...
  enviarEmailRhAtivar() {
    ...
    this.siscrhService.enviarEmailRhAtivar(this.IDColab).subscribe(
      (data: any) => {
        this.toastr.success('Solicitação enviada com sucesso!', 'Atenção!');
      },
      (error) => {
  ...
  enviarEmailInformaticaAtivo() {
    ...
    this.siscrhService.enviarEmailInformaticaAtivo(this.IDColab).subscribe(
      (data: any) => {
  ...
  enviarEmailInformaticaDesativo() {
    ...
    this.siscrhService.enviarEmailInformaticaDesativo(this.IDColab).subscribe(
      (data: any) => {
        this.toastr.success('E-Mail enviado com sucesso!', 'Atenção!');
      },
      (error) => {
  ...
  upload(event: any) {
    if (event.target.files && event.target.files[0]) {
      ...
      this.siscrhService
        .uploadFotoColaborador(formData, this.IDColab)
        .subscribe(
          (data: any) => {
            console.log(data);
